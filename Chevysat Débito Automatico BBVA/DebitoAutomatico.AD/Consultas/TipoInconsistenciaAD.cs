...
{
    public class TipoInconsistenciaAD
    {
        public String Error { get; set; }

        public ILog Registrador { get; set; }

        public TipoInconsistenciaAD()
        {
            Registrador = LogManager.GetLogger(this.GetType());
        }


        protected DataSet ejecutarConsulta(TipoInconsistencia objEntidad)
        {
            ...
            try
            {
                ...
                adaptador = new SqlDataAdapter("pa_DEB_Tipo_Inconsistencia", conexion);
        ...
        public List<TipoInconsistencia> consultar(TipoInconsistencia objEntidad)
        {
            DataSet datos = ejecutarConsulta(objEntidad);

            List<TipoInconsistencia> lista = new List<TipoInconsistencia>();
            TipoInconsistencia objEntidad2 = null;
            foreach (DataRow fila in datos.Tables["tabla"].Rows)
            {
                objEntidad2 = new TipoInconsistencia();
        ...
        public int ejecutarNoConsulta(TipoInconsistencia objEntidad)
        {
            int cuenta = -1;
            DataSet datos = ejecutarConsulta(objEntidad);
            try
